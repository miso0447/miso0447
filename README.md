...
## 🤖 AI 수학 튜터 챗봇 (OpenAI 연동)

- **중학수학 특화 AI 선생님**: 중학교 1~3학년 학생의 눈높이에 맞춰 친절하고 단계별로 수식과 함께 설명해주는 대화형 챗봇입니다.
- **KaTeX 수식 렌더링**: $y = ax + b$, $a^2 + b^2 = c^2$, $\sin 30^\circ = \frac{1}{2}$ 등 복잡한 수식이 교과서처럼 깔끔하게 표시됩니다.
- **환경변수 `OPENAI_API_KEY` 연동**:
  - Vercel 대시보드의 **Settings → Environment Variables**에 `OPENAI_API_KEY`를 등록하면, 서버리스 엔드포인트(`/api/chat.js`)를 통해 API 키가 클라이언트에 노출되지 않고 안전하게 통신합니다.
  - 로컬 테스트 환경에서도 챗봇 우측 상단의 `[⚙️ API 설정]` 버튼을 통해 브라우저 로컬 저장소에 키를 직접 입력하여 테스트할 수 있습니다.
...
1. GitHub 저장소를 Vercel에 임포트(Import)합니다.
2. Vercel 프로젝트 대시보드의 **Settings → Environment Variables**에 아래 환경변수를 등록합니다:
   - `OPENAI_API_KEY`: 발급받은 OpenAI API 키 (`sk-...`)
3. Git `push` 시 Vercel에서 수초 내에 자동 배포되며, AI 튜터 챗봇과 모든 기능이 즉시 활성화됩니다.
